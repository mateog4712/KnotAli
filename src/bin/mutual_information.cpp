...
string MIVector(vector<string> seqs){

// Initialize our vector of pairs with score > threshold
...
for(int i = 0; i<n;++i){
    for(int j = 4;(j<30 && (i+j)<n);++j){
        double score = calcMutualInformation(&cols[i][0],&cols[i+j][0], n_seq); 
        if(score > 1) {
            Hotspot hotspot;
...
string MIVectorStack(vector<string> seqs){
...
for(int i = 0; i<n;++i){
    for(int j = 4;(j<30 && (i+j)<n);++j){
        ...
        if(i-1 <=0 || i+j+1 >= n){
          ...
          prev = calcMutualInformation(&cols[i-1][0],&cols[i+j+1][0], n_seq);
        }
        if(i+1 >=n && j-i-1>4){
          foll = score;
        }
        else{
          foll = calcMutualInformation(&cols[i+1][0],&cols[i+j-1][0], n_seq);
        }
