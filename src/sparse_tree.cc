 ...
 */
int sparse_tree::query(int l,int r) const{
    int d = r-l;
    int dx = logn[d];
    if (l==r) return l;
...
*/
int sparse_tree::LCA(int i,int j) const{
            // trivial case
    if (i==j) return i;
...
*/
int sparse_tree::bp(int i, int l ) const{
...
*/
int sparse_tree::Bp(int l, int j) const{
    if(tree[l].parent->index == 0 || tree[l].pair > -1) return -2;
...
*/
int sparse_tree::B(int l, int j) const{
    if(tree[l].parent->index == 0 || tree[l].pair > -1) return -2;
    if ((tree[l].parent)->pair > j) return -1;
    else{
        int lca = LCA(l,j);
...
int sparse_tree::b(int i, int l) const{
    if(tree[l].parent->index == 0 || tree[l].pair > -1) return -2;
    if ((tree[l].parent)->index < i) return -1;
    else{
        int lca = LCA(i,l);
...
*/
bool sparse_tree::weakly_closed(int i, int j) const{
    if(j<i) return 0;
