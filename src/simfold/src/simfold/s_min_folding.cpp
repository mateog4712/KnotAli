...
double s_min_folding::s_simfold_new (double *scores, int length)
// PRE:  the init_data function has been called;
//       the space for structure has been allocated
// POST: fold sequence, return the MFE structure in structure, and return the MFE
{
    double energy;
    energy = fold_sequence_new (scores, length);
    return energy;
}
...
double s_min_folding::fold_sequence_new (double *scores, int n)
// helper function, it folds sequence, returns structure and free energy
{
    double energy;
    int i, j;
    ...
    for (j=0; j < nb_nucleotides; j++)
    {
        ...
        {
            // if (constraints[i]) continue;
            V->compute_energy_new (i,j,scores,n);
        }
        // if I put this before V calculation, WM(i,j) cannot be calculated, because it returns infinity
        VM->compute_energy_WM (j);
    }
    for (j=1; j < nb_nucleotides; j++)
    {
        compute_W (j); //add the new version when done
    }
    energy = W[nb_nucleotides-1]/100.0;
...
//std::cout << cur_interval->type << std::endl;
    char type;
    ...
    if(cur_interval->type == LOOP)
    {
        ...
        else if (type == INTER)
        {
            ...
            int ip = 0, jp=0, best_ip=0, best_jp=0, minq=0;
            PARAMTYPE tmp = 0, min = INF;
            // changed <= to <
            //std::cout<< "This is maxloop " << MAXLOOP << " " << i+MAXLOOP+1<<std::endl;
            for (ip = i+1; ip < MIN(j-2,i+MAXLOOP+1); ip++)
            {
            /** Mateo 2020 changed to match Hosna 2012 change **/
               // minq = MAX (j-i+ip-MAXLOOP-2, ip+1);
               
               minq = ip + 1 + TURN;
                //std::cout << i << " " << j << " " << ip << " " << minq << std::endl;
                for (jp = minq; jp < j; jp++)
                {
            ...
            {
                //assert(0);
                fprintf (stderr, "NOT GOOD RESTR INTER, i=%d, j=%d,ip=%d,jp=%d best_ip=%d, best_jp=%d\n", i, j,ip,jp, best_ip, best_jp);
                exit (0);
            }
        }
        else if (type == MULTI)
