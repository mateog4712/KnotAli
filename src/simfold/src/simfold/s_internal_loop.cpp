...
// 3) ip.jp is enforced in the restricted structure and we have i.j canonical but ip.jp is non-canonical
{
    ...
    if ((sequence[ip] + sequence[jp] == 3 || sequence[ip] + sequence[jp] == 5) &&
        can_pair(sequence[i], sequence[j])) // normal case
    {
        ...
        if (branch1 != 0 || branch2 != 0) {
            ...
            if (branch1 == 1 && branch2 == 1 && !simple_internal_energy)     // it is int11
            {
                ...
                [sequence[ip]][sequence[jp]];
                en = MIN(0,
                         en); // Hosna, March 26, 2012, added to accommodate non-cannonical base pairs in restricted structure
...
// This function is just most of what is inside the second for loop of compute_energy
{
    ...
    if (sequence[ip] + sequence[jp] == 3 || sequence[ip] + sequence[jp] == 5) {
        ...
        branch1 = ip - i - 1;
        branch2 = j - jp - 1;
        //std::cout << "b1 b2 " << branch1 << " " << branch2 << std::endl;
        if (branch1 != 0 || branch2 != 0) {
            // check if it is a bulge loop of size 1
            // check if it is int11 or int21 or int22
            if (branch1 == 1 && branch2 == 1 && !simple_internal_energy)     // it is int11
            {
                ...
                if (branch1 == 0 || branch2 == 0) {
                    ...
                    if ((branch1 == 1 || branch2 == 1) && misc.gail_rule)
                        ...
                        // i_j_energy and ip_jp_energy will be calculated as if it was a loop of As
                    {
                        //std::cout << "inside misc.gail" << std::endl;
//#if (MODEL == SIMPLE)
                        // In the simple model I only use 3 parameters for tstacki,
                        //  So tstacki[i][j][[0][0] never comes up, I just ignore it
                        //i_j_energy  =  tstacki[sequence[i]][sequence[j]][0][0];
...
//#endif
                    ...
                    ttmp = i_j_energy + ip_jp_energy + penalty_size +
                           asym_penalty + V->get_energy(ip, jp);
                    //std::cout<< " ttmp" << i_j_energy << " " << ip_jp_energy << " " << penalty_size << " "
                    //<< asym_penalty << " " << V->get_energy(ip,jp) << std::endl;
                    if (ttmp < mmin) {
                        mmin = ttmp;
                    }
                }
...
// Also called by the partition function
{
    ...
    if (branch1 != 0 || branch2 != 0) {
        ...
        if (branch1 == 1 && branch2 == 1 && !simple_internal_energy)     // it is int11
        {
            ...
            [sequence[i + 1]][sequence[j - 1]]
            [sequence[ip]][sequence[jp]];
            check_int11_parameters(sequence[i], sequence[j], sequence[i + 1], sequence[j - 1], sequence[ip],
                                   sequence[jp]);
        } else if (branch1 == 1 && branch2 == 2 && !simple_internal_energy) {
            // int21[i][j][i+1][j-1][ip][jp][jp+1]
...
//     }
    ...
    if (parsi_int11 == PARSI || creating_model ||
        ...
        if (parsi_int11 == LAVISH || parsi_int11 == HLI) {
            ...
            if (ii * 100000 + jj * 10000 + kk * 1000 + ll * 100 + mm * 10 + nn <=
                nn * 100000 + mm * 10000 + ll * 1000 + kk * 100 + jj * 10 + ii) {
                if (creating_model)
        ...
        if (parsi_int22 == LAVISH) {
            ...
            if (ii * 10000000 + jj * 1000000 + kk * 100000 + ll * 10000 + mm * 1000 + nn * 100 + oo * 10 + pp <=
                ...
                if (creating_model)
                    ...
                    sprintf(type, "int22_experimental_addition[%d][%d][%d][%d][%d][%d][%d][%d]", ii, jj, kk, ll, mm, nn,
                            oo, pp);
            } else {
                if (creating_model)
                    sprintf(type, "int22[%d][%d][%d][%d][%d][%d][%d][%d]", nn, mm, pp, oo, jj, ii, ll, kk);
                else
                    sprintf(type, "int22_experimental_addition[%d][%d][%d][%d][%d][%d][%d][%d]", nn, mm, pp, oo, jj, ii,
                            ll, kk);
            }
            index = structure_type_index(type);
            counter[index]++;
...
// Mirela: Nov 23, 2003
{
    ...
    if (branch1 != 0 || branch2 != 0) {
        ...
        if (branch1 == 1 && branch2 == 1 && !simple_internal_energy)     // it is int11
        {
            ...
            if (sequence[i] * 100000 + sequence[j] * 10000 + sequence[i + 1] * 1000 + sequence[j - 1] * 100 +
                sequence[ip] * 10 + sequence[jp] >
                sequence[jp] * 100000 + sequence[ip] * 10000 + sequence[j - 1] * 1000 + sequence[i + 1] * 100 +
...
//                            [sequence[jp+1]]);
            ...
            if (parsi_int21 == T99) {
                if ((ii == C && jj == G && mm == C && nn == G) ||
                    // these are already filled above, except what can pair inside
                    (ii == G && jj == C && mm == G && nn == C)) {
...
//                            [sequence[ip-1]][sequence[jp+1]]);
            ...
            if (sequence[i] * 10000000 + sequence[j] * 1000000 + sequence[i + 1] * 100000 + sequence[j - 1] * 10000 +
                ...
                sequence[jp] * 10000000 + sequence[ip] * 1000000 + sequence[jp + 1] * 100000 +
                sequence[ip - 1] * 10000 +
            ...
            if (parsi_int22 == T99) {
                ...
                } else if (((ii == G && jj == U) || (ii == U && jj == G) || (mm == G && nn == U) ||
                            (mm == U && nn == G)) &&
                ...
                if (l == 1) {
                    if (parsi_bulge1 == T99) {
                        ...
                        if (1000 * sequence[i] + 100 * sequence[j] + 10 * sequence[ip] + sequence[jp] >
                            ...
                            sprintf(type, "stack[%d][%d][%d][%d]", sequence[jp], sequence[ip], sequence[j],
                                    sequence[i]);
                            energy += stack[sequence[jp]][sequence[ip]][sequence[j]][sequence[i]];
                        } else {
                            sprintf(type, "stack[%d][%d][%d][%d]", sequence[i], sequence[j], sequence[ip],
                                    sequence[jp]);
...
//#endif
                    ...
                    if (parsi_tstacki == T99 || parsi_tstacki == PARSI) {
                        ...
                        sprintf(type, "tstacki[%d][%d][%d][%d]", sequence[i], sequence[j], sequence[i + 1],
                                sequence[j - 1]);
                        index = structure_type_index(type);
                        counter[index]++;
                    ...
                    if (parsi_tstacki == T99 || parsi_tstacki == PARSI) {
                        ...
                        if (parsi_tstacki == T99) {
                            if ((sequence[jp + 1] == A && sequence[ip - 1] == G) ||
                                (sequence[jp + 1] == G && sequence[ip - 1] == A)) {
                                sprintf(type, "misc.internal_GA_AG_mismatch");
                                index = structure_type_index(type);
                                counter[index]++;
                                energy += misc.internal_GA_AG_mismatch;
                        ...
                        // SHOULD NOT apply rule 1 here, they are separate parameters
                        sprintf(type, "tstacki[%d][%d][%d][%d]", sequence[jp], sequence[ip], sequence[jp + 1],
                                sequence[ip - 1]);
                        index = structure_type_index(type);
                        counter[index]++;
