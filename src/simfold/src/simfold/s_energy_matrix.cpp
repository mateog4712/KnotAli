...
void s_energy_matrix::compute_energy_new (int i, int j, double *scores, int n)
// compute the V(i,j) value
{

    PARAMTYPE min, min_en[4], min_en_new[4];
    int k, min_rank;
    char type;
    ...
    if (can_pair (sequence[i], sequence[j]))    // if i and j can pair
    {
        // compute free energy of hairpin loop, stack pair, internal loop and multi-loop
        min_en[0] = H->compute_energy (i, j);
        min_en_new[0] = min_en[0] - scores[i*n+j]/100;
        if (i<=j-TURN-1)
        {
            min_en[1] = S->compute_energy (i, j);
            min_en_new[1] = min_en[1]- scores[i*n+j]/100;
            // TODO: uncomment
            if (!ignore_internal)
                min_en[2] = VBI->compute_energy (i, j);
                min_en_new[2] = min_en[2]- scores[i*n+j]/100;
            if (!ignore_multi)
                min_en[3] = VM->compute_energy (i, j);
                min_en_new[3] = min_en[3]- scores[i*n+j]/100;
        }
    }
